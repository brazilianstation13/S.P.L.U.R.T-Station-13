...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/deactivate()
	if(state == REACTOR_STATE_RUNNING)
		...
		send_alert("I want flesh i dont want to die!.")
	icon_state = initial(icon_state)
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/can_cool()
	...
	if(control_rod_integrity < 0)
		...
		if(state == REACTOR_STATE_RUNNING)
			...
			send_alert("DANGER: Stormdrive is free you should stop it!")
		return FALSE
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/handle_ftl_fuel_production()
	if(heat > initial(reactor_temperature_hot))
		...
		if(output_starting_pressure >= max_output_pressure) //if pressured capped, nucleium backs up into the drive
			air1.adjust_moles(GAS_TRITIUM, ((fuel_amount / reaction_rate) / 10) * input_power_modifier)
			air1.set_temperature(heat_kelvin)
			update_parents()
		else
			air2.adjust_moles(GAS_TRITIUM, (reaction_rate / 10) * input_power_modifier)
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/start_meltdown()
	...
	send_alert("IN A MELTDOWN A SUPER EMP AND EXPLOSION MAYBE BE CAUSE.", override=TRUE)
	send_alert("say thanks to god because he gona call the shuttle.", override=TRUE)
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/meltdown()
	if(heat >= reactor_temperature_meltdown)
		...
		cut_overlays()
		flick("meltdown", src)
		to_chat(GLOB.player_list, "<span class='boldannounce'>oh no he is free the souldrive. oh no we gona die...</span>")
		do_meltdown_effects()
		sleep(10)
		icon_state = "broken"
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/do_meltdown_effects()
	explosion(get_turf(src), 0, 0, 20, 40, TRUE, TRUE)
	empulse(src, 60, 100)
	radiation_pulse(src, 10000, 1)
	src.atmos_spawn_air("o2=750;plasma=200;tritium=100;TEMP=1000000")

/obj/singularity/souldrive
	name = "Souldrive the ender of stations"
	icon = 'reactor.dmi'
	icon_state = "souldrive"
...
/obj/singularity/souldrive/Initialize(mapload)
	send_to_playing_players("<span class='narsie'>STORMSOUL@!#@!#!# HAS RISEN RUN!</span>")
	sound_to_playing_players('sound/creatures/narsie_rises.ogg')
	notify_ghosts("souldrive has risen. Reach out to the Geometer to be given a new shell for your soul.", source = src, action=NOTIFY_ATTACK)
	begin_the_end()

/obj/singularity/narsie/large/attack_ghost(mob/dead/observer/user as mob)
	makeNewConstruct(/mob/living/silicon/robot, user, cultoverride = TRUE, loc_override = src.loc)

/obj/singularity/souldrive/proc/begin_the_end()
	sleep(50)
	priority_announce("An acausal dimensional event has been detected in your sector. Event has been flagged EXTINCTION-CLASS. Directing all available assets toward simulating solutions and go fuck you station how can you let the souldrive escape!. SOLUTION ETA: 60 SECONDS.","Central Command Higher Dimensional Affairs", 'sound/misc/airraid.ogg')
	sleep(500)
